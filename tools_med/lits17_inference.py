...
DEFAULT_HU_MIN = -200.0
DEFAULT_HU_MAX = 250.0
...
def infer_case_id(nifti_path):
    ...
    name = re.sub(
        r"^(volume-|ct-|CT-|case-|Case-)",
        "",
        name
    )

    return "LiTS_" + name if not name.startswith("LiTS_") else name


def normalize_ct(volume, hu_min=-200.0, hu_max=250.0):
    """
    LiTS17 CT preprocessing.

    Same logic as training preprocess:
    - clip HU to liver window
    - scale to [0, 1]
    """
...
def build_model(config_path, checkpoint_path, device="cuda:0"):
    ...
    # Important for mmseg.apis.inference_segmentor
    model.cfg = cfg
...
def run_slice_inference(model, image_paths, pred_dir):
    ...
    LiTS17 tumor-only binary:
        pred == 0: background / non-tumor
        pred > 0 : tumor
...
def compute_volume_metrics(pred_mask, nifti_img):
    """
    Compute predicted tumor volume using original NIfTI spacing.
    """
...
def save_metadata_csv(metrics, out_path):
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Clinical-style end-to-end inference pipeline for LiTS17 CT liver tumor "
    ...
    parser.add_argument(
        "--out-root",
        default="work_dirs/lits17/infer",
    ...
    metrics["target"] = "liver_tumor_only"
    ...
    print("\n========== LiTS17 CLINICAL INFERENCE SUMMARY ==========")
    ...
    print("Voxel volume:")
    print("{:.6f} mm3".format(metrics["voxel_volume_mm3"]))
    print("Pred tumor volume:")
    print("{:.2f} mm3".format(metrics["pred_volume_mm3"]))
    print("Pred tumor volume:")
    print("{:.4f} mL".format(metrics["pred_volume_ml"]))
