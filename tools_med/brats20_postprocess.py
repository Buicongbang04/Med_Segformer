...
def load_binary_mask(path):
    """
    Load PNG mask as binary mask.
    Both are converted to bool by mask > 0.
    """
...
def group_slices(folder):
    """
    Return dict: case_id -> list of (slice_index, file_path) for all slices in that case.
    """
...
def dice_score(pred, gt, eps=1e-8):
    ...
    # If both prediction and GT are empty, consider it perfect.
    if denom == 0:
        return 1.0
...
def run_mmseg_prediction(config, checkpoint, pred_dir, clean_pred_dir=True):
    """
    Run MMSeg inference exactly like tools/test.py,
    then save binary prediction masks to:
    ...
    for pred, img_info in zip(outputs, dataset.img_infos):
        ...
        # print(f"[DEBUG] {fname}: unique pred values = {unique_vals[:20]}")
...
def evaluate_3d(pred_dir, gt_dir, voxel_volume_mm3, require_contiguous=True):
    ...
    if missing_pred:
        raise RuntimeError(f"Missing predictions for {len(missing_pred)} cases: {missing_pred[:10]}")
    if missing_gt:
        raise RuntimeError(f"Missing GT for {len(missing_gt)} cases: {missing_gt[:10]}")

    rows = []

    for case_id in sorted(gt_ids):
        pred_raw = stack_volume(case_id, pred_cases[case_id], require_contiguous=require_contiguous)
        gt = stack_volume(case_id, gt_cases[case_id], require_contiguous=require_contiguous)
...
def summarize(df):
    finite_raw_rel = df["raw_relative_volume_error"].replace([np.inf, -np.inf], np.nan)

    summary = {
        "num_cases": int(len(df)),
        "mean_raw_3d_dice": float(df["raw_3d_dice"].mean()),
        "std_raw_3d_dice": float(df["raw_3d_dice"].std()),
        "mean_raw_abs_volume_error_mm3": float(df["raw_abs_volume_error_mm3"].mean()),
...
def main():
    parser = argparse.ArgumentParser(
        description="Run MMSeg checkpoint prediction on BraTS20 test set, reconstruct 3D volumes, compute 3D Dice and tumor volume metrics."
    )
    parser.add_argument("--config", required=True, help="MMSeg config file.")
    parser.add_argument("--checkpoint", required=True, help="Checkpoint .pth to evaluate.")
    parser.add_argument("--gt-dir", required=True, help="Ground-truth annotation folder for test split.")
    parser.add_argument("--pred-dir", required=True, help="Directory to save/read MMSeg predictions.")
    parser.add_argument("--out-csv", required=True, help="Output CSV path for per-case 3D results.")
    parser.add_argument("--voxel-volume-mm3", type=float, default=1.0, help="Voxel volume in mm^3. BraTS is often 1.0 after resampling.")
    parser.add_argument("--skip-predict", action="store_true", help="Skip MMSeg prediction and only evaluate existing pred-dir.")
    parser.add_argument("--no-clean-pred-dir", action="store_true", help="Do not remove pred-dir before prediction.")
